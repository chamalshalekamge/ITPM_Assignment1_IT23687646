        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            try:
                ...
                if action_locator:
                    ...
                    # Wait for the button to return from "Transliterating..." to "Transliterate"
                    # This signals the API call has completed
                    try:
                        page.wait_for_function(
                            "() => { const b = document.querySelector('button.btn'); return b && b.innerText.trim() === 'Transliterate'; }",
                            timeout=max(1000, int(args.wait_ms) * 6)
                        )
                    except Exception:
                        pass
